...
**Repositorio en GitHub:** [https://github.com/decheverriae-prog/undo-and-redo](https://github.com/decheverriae-prog/undo-and-redo)  
...
4. Use **Undo** y **Redo**; el historial y el estado actual se muestran en pantalla.

## Nota sobre el informe en PDF

El texto listo para copiar al entregable en PDF está en [`docs/INFORME_PDF.md`](docs/INFORME_PDF.md). Complete el **enlace de Google Drive** del video y exporte a PDF; el enlace de GitHub ya figura allí y en este README (ajústelo si el nombre del repo en GitHub no coincide con `undo-and-redo`).

## Publicar en GitHub

1. Inicie sesión en GitHub con la cuenta correcta.
2. **Cree el repositorio vacío** en la web: **New repository** → nombre `undo-and-redo` → **Public** → **sin** marcar “Add a README” (el proyecto ya trae archivos).
3. Compruebe en el navegador que abre sin error:  
   `https://github.com/TU_USUARIO/undo-and-redo`  
   Si da **404**, el repo **aún no existe** o el **usuario** no coincide con `TU_USUARIO`.
4. En la carpeta del proyecto:

```powershell
git remote set-url origin https://github.com/TU_USUARIO/undo-and-redo.git
